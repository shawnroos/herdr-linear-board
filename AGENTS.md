...
schema v15
live catalog is `e2e/README.md` (scenarios 01–41); `e2e/test-harness.sh` is the provider-free static
safety gate.
...
- End-to-end: `e2e/run-all.sh` (compat: `scripts/e2e.sh`) drives a REAL Herdr; checked-in fake
  Pi/Claude/Codex/OpenCode/Antigravity (`agy`) executables keep the standard suite (scenarios 01–41) provider-free and zero-cost, except `41-linear-bind-handoff.sh`, which starts the real `claude` and skips without it.
