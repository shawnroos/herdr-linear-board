...
| `board linear issue <ws> <identifier>` runs the work plugin's `bin/work-issue.sh` (a stand-in root at the version floor) and returns the whole issue — description, sub-issues, parent, relations, comments and history — in ONE Linear call; a plugin that ships no issue script is protocol code 7 / exit 7 ("update the plugin") while `linear snapshot` on the same root still works, and a plugin below the version floor is still code 6; an unreachable Linear is carried in the document rather than raised, and a paged connection that was cut names what it cut | `42-linear-issue.sh` | live, provider-free; no Linear call |
...
scenarios 01–42 (41 excepted: it starts the real `claude`) without provider calls; this is a coverage requirement, not a claim that a live run
...
| `42-linear-issue.sh` | Per-issue read: drives `board linear issue` against a stand-in plugin serving the vendored `crates/board-core/tests/fixtures/linear-issue/` documents; asserts the one-call shape, the full and empty documents, the `unavailable` and truncated `partial` statuses, and the distinction the design rests on — a missing issue script exits 7 while `linear snapshot` on the same root keeps working, and a plugin under the version floor still exits 6 (floor read from `PLUGIN_VERSION_FLOOR`). |
...
| `run-all.sh` | Builds once, runs scenarios 01–42 as environment-scrubbed children with their own sessions, captures artifacts, and prints the summary (`--require-all` forbids skips). | with their own sessions, captures artifacts, and prints the summary (`--require-all` forbids skips). |
